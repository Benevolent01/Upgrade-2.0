...
async function moderateChat(msg) {
  ...
  try {
    ...
    await msg.member
      .timeout(timeout_duration, "They deserved it.")
      .then(async () => {
        msg.delete();
        msg.channel.send(
          `${msg.author.username} got timed out for ${hours} hour${
            hours > 1 ? "s" : ""
          }!`
        );
      })
      .catch((e) => {
        console.log(e);
        msg.channel.send(
          "I'm sure you said a bad word, but I can't time out out?!"
        );
      });
  } catch (e) {
    console.log(e);
